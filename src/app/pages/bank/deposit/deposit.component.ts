    ...
    this.customerService.getAllCustomers().subscribe(
      res => {
        ...
        if (res != null) {
          ...
          const router = this.injector.get(Router);
          router.navigate(['/bank']);
          swal('Mensaje!', `No existen Clientes favor de crear uno`, 'warning');
        }
      }
    );
...
console.log('Valor de i ', i);
  ...
  filterCustomer() {
    ...
    this.filteredCustomerOptions = this.form.get('customer').valueChanges
    .pipe(
      startWith<string | Customer>(''),
      map(value => typeof value === 'string' ? value : value.nameCustomer),
  ...
  displayCustomerFn(supp?: Customer): string | undefined {
    return supp ? supp.nameCustomer : undefined;
  }
  ...
  //  this.suppliers = this.form.get('supplier').value;
    const filterCustomerValue = name.toLowerCase();
    return this.customers.filter(option => option.nameCustomer.toLowerCase().indexOf(filterCustomerValue) === 0);
  }
  ...
  createForm() {
    ...
    addPolizaAutom() {
      ...
      || this.form.get('customer').invalid || this.form.get('depositAmount').invalid
      ...
      let cheque = this.form.get('depositAmount').value;
    ...
    addPolizaManual() {
       ...
       const cheque = this.form.get('depositAmount').value;
